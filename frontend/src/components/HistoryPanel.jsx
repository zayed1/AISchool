...
function HistoryPanel({ onSelect }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      items = items.filter((item) => item.preview?.toLowerCase().includes(q))
    }
    // Filter by color
    if (filterColor !== 'all') {
      ...
      {isOpen && (
        <div className="border-t border-slate-100 dark:border-slate-700">
          ...
          <div className="px-4 py-2 border-t border-slate-100 dark:border-slate-700 flex justify-between items-center">
            <button onClick={clearHistory} className={`text-xs transition-colors ${confirmClear ? 'text-red-600 dark:text-red-400 font-bold' : 'text-red-400 hover:text-red-600 dark:text-red-500 dark:hover:text-red-400'}`}>
              {confirmClear ? 'اضغط مرة أخرى للتأكيد' : 'مسح السجل'}
            </button>
