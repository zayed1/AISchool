          ...
          <div className="max-h-48 overflow-y-auto">
            {history.map((item, index) => (
              <button
                ...
                className="w-full text-right px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-700 border-b border-slate-50 dark:border-slate-700 transition-colors"
              >
                <div className="flex items-center gap-2">
                  <span className={`w-2.5 h-2.5 rounded-full shrink-0 ${colorDot[item.result.color] || 'bg-slate-300'}`} aria-hidden="true" />
                  <span className="text-sm text-slate-700 dark:text-slate-300 truncate flex-1">{item.preview}</span>
                  <span className="text-xs text-slate-400 dark:text-slate-500 shrink-0">{item.result.percentage}%</span>
                </div>
                <div className="flex items-center gap-2 mt-1 mr-4">
                  <span className="text-xs text-slate-400 dark:text-slate-500">{item.metadata.word_count} كلمة</span>
