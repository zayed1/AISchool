...
function UsageBadge({ onLogin, onPricing }) {
  ...
  if (!isAuthenticated) {
    return (
      <div className="flex items-center gap-2">
        {/* Show free usage even for anonymous */}
        <span className="text-[10px] text-slate-400 dark:text-slate-500 hidden sm:inline">{remaining}/{limit}</span>
  ...
  return (
    <div className="flex items-center gap-2">
      {/* Usage indicator */}
      <button onClick={onPricing} className="flex items-center gap-1.5 text-xs" title={`${used}/${limit} تحليل اليوم — متبقي ${remaining}`}>
        <div className="w-14 h-1.5 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden">
          <div className={`h-full rounded-full transition-all duration-300 ${isLow ? 'bg-red-400' : 'bg-primary-400'}`} style={{ width: `${usagePercent}%` }} />
        </div>
        <span className={`font-medium tabular-nums ${isLow ? 'text-red-500' : 'text-slate-500 dark:text-slate-400'}`}>{remaining}</span>
      ...
      <div className="relative group">
        ...
        <div className="absolute left-0 top-full mt-1 hidden group-hover:block z-50">
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 py-1 w-40" dir="rtl">
            ...
            <p className="px-3 py-1 text-[10px] text-slate-500">{used}/{limit} تحليل اليوم</p>
